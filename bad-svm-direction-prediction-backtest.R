...
# Back-testing a machine learning strategy that retrains every day
...
addFewerFeatures <- function(data) {
  ...
  # other features  
  res <- merge(res, 
               xts(na.trim(lag(rollmean(returns, k = 21, align = "right"), 
                              1))),  
               xts(na.trim(lag(rollmedian(returns, k = 21, align = "right"), 
                               1))), 
               xts(na.trim(lag(apply.rolling(returns, width = 21, FUN = sd), 
                               1))), 
               xts(na.trim(lag(apply.rolling(returns, width = 21, FUN = mad), 
                               1))), 
               xts(na.trim(lag(apply.rolling(returns, width = 21, 
                                             align = "right", FUN = skewness), 
                               1))), 
               xts(na.trim(lag(apply.rolling(returns, width = 21,  
                                             align = "right", FUN = kurtosis), 
                               1))),  
               all = FALSE)
  # add volume
